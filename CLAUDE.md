...
- `tools/build_caption_prompt_from_taxonomy.py` — converts WeDetect-style medical taxonomy ([data/medical/tct_ngc_taxonomy.json](data/medical/tct_ngc_taxonomy.json)) into GLIP-compatible prompts so the language encoder sees medically-precise descriptions (PSC / Bethesda / Paris diagnostic systems) instead of bare `"organ-shortname"` labels. Four description sources: `diag_raw` (richest, e.g. *"PSC Category II: Negative — acute neutrophilic inflammation"*), `diag_full` (short tag like *"PSC II Negative"*), `diag_entity` (cell entity only), or `fullnames` (hand-curated descriptive prompts in [data/medical/tct_ngc_fullnames_30.json](data/medical/tct_ngc_fullnames_30.json) / [data/medical/tct_ngc_fullnames_32.json](data/medical/tct_ngc_fullnames_32.json)). Emits GLIP `OVERRIDE_CATEGORY` JSON (for inline `DATASETS.OVERRIDE_CATEGORY` in config + `DATASETS.USE_OVERRIDE_CATEGORY True`) and/or `caption_prompt` YAML (for `DATASETS.CAPTION_PROMPT` + `DATASETS.USE_CAPTION_PROMPT True`). Fullnames lookup is by class **name** via `taxonomy.class_id`, not positional — survives any reordering of the dataset's categories.
...
- **Silent BERT truncation when class prompts are long.** GLIP defaults to concatenating ALL category names into one BERT query (`TEST.CHUNKED_EVALUATION = -1`, `MAX_QUERY_LEN = 256`, `truncation=True`). With descriptive medical prompts (`data/medical/prompts/tct_ngc_base30.*.override.json`), the 30-class concat hits **389–660 BERT tokens**, so anywhere from the **last 7 to the last 18 classes are silently dropped** from the language input and become un-detectable. Always run `python tools/check_prompt_token_length.py <override.json>` before eval — it prints the recommended `TEST.CHUNKED_EVALUATION K` value (e.g. `K=11` for `diag_raw`, `K=20` for `diag_entity`, `K=23` for `diag_full`, `-1` is fine for raw class names at 231 tokens). Pass it as a CLI yacs override: `TEST.CHUNKED_EVALUATION 11`. A ready-to-go task config wiring the `diag_raw` override + `CHUNKED_EVALUATION 11` is in [configs/medical/tct_ngc_base30_diag_raw.yaml](configs/medical/tct_ngc_base30_diag_raw.yaml); see [configs/medical/README.md](configs/medical/README.md) for the per-format chunk table and how to swap prompt formats on the CLI.
